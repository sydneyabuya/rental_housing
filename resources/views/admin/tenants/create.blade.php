                ...
                <form method="post" action="{{ route('tenants.store') }}">
                    ...
                    <div class="shadow overflow-hidden sm:rounded-md">
                        ...
                        <div class="px-4 py-5 bg-white sm:p-6">
                            ...
                            <input type="number" name="user_id" id="user_id" class="form-input rounded-md shadow-sm mt-1 block w-full"
                                   value="{{ old('user_id', '') }}" />
                            @error('user_id')
                                <p class="text-sm text-red-600">{{ $message }}</p>
                            @enderror
                        ...
                        <div class="px-4 py-5 bg-white sm:p-6">
                            ...
                            <select name="user[]" id="user_id" class="form-multiselect block rounded-md shadow-sm mt-1 w-full" multiple="multiple">
                                @foreach($users as $id => $user)
                                    <option value="{{ $id }}"{{ in_array($id, old('user_id', [])) ? ' selected' : '' }}>{{ $user }}</option>
                                @endforeach
                            </select>
                            @error('users')
                                <p class="text-sm text-red-600">{{ $message }}</p>
