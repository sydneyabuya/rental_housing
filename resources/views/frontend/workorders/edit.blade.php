    ...
    <div class="row justify-content-center">
        <div class="col-md-12">
            ...
            <div class="card">
                ...
                <div class="card-body">
                    <form method="POST" action="{{ route("frontend.workorders.update", [$workorder->id]) }}" enctype="multipart/form-data">
                        @method('PUT')
                        @csrf
                        <div class="form-group">
                            ...
                            <select class="form-control select2" name="vendor_id" id="vendor_id" required>
                                @foreach($vendors as $id => $entry)
                                    <option value="{{ $id }}" {{ (old('vendor_id') ? old('vendor_id') : $workorder->vendor->id ?? '') == $id ? 'selected' : '' }}>{{ $entry }}</option>
                                @endforeach
                            </select>
                        ...
                        <div class="form-group">
                            ...
                            <select class="form-control select2" name="vendor_spec_id" id="vendor_spec_id">
                                @foreach($vendor_specs as $id => $entry)
                                    <option value="{{ $id }}" {{ (old('vendor_spec_id') ? old('vendor_spec_id') : $workorder->vendor_spec->id ?? '') == $id ? 'selected' : '' }}>{{ $entry }}</option>
                                @endforeach
                            </select>
                        ...
                        <div class="form-group">
                            ...
                            <select class="form-control select2" name="details_id" id="details_id" required>
                                @foreach($details as $id => $entry)
                                    <option value="{{ $id }}" {{ (old('details_id') ? old('details_id') : $workorder->details->id ?? '') == $id ? 'selected' : '' }}>{{ $entry }}</option>
                                @endforeach
                            </select>
